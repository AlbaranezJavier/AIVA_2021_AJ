...
from tools.LabelsManager import read_yaml
...
class Metrics:
    ...
    def cal_basic_stats_modified(self, predicted, gt, path):
        """
        Calculates tp, fn, fp and tn for a predicted mask and its ground thruth
        :param predicted: mask
        :param gt: mask
    ...
    def add_cumulative_stats(self, tp, fn, fp, tn):
        """
    ...
    def iou(self, basic_type):
        try:
            self.stats["iou"] = round((self.basic_stats[basic_type]["tp"] / (self.stats["population"] - self.basic_stats[basic_type]["tn"])) * 100, 2)
    ...
    def f1_score(self, basic_type):
        try:
            self.stats["f1"] = round((2 * self.basic_stats[basic_type]["tp"] / (self.stats["rp"] + self.stats["pp"])) * 100,
                                     2)
